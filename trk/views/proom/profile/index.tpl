...
<div class="profile-tab">
    ...
    <fieldset>
        <span style="color:red; font-weight:bold">{$lng_label_prifle_legend}:</span>
        <br/>
        <table class="photograper-data">
            <tr>
                <td colspan="2">{$trk_user_userId_hidden}</td>
            </tr>
            <tr>
                <th>{$lng_label_prifle_photograper_first_name}</th>
                <td>{$trk_user_sys_user_name_input}</td>
            </tr>
            <tr>
                <th>{$lng_label_prifle_photograper_last_name}</th>
                <td>{$trk_user_sys_user_lastname_input}</td>
            </tr>
            <tr>
                <th>{$lng_label_prifle_photograper_email}</th>
                <td>{$trk_user_sys_user_email_input}</td>
            </tr>
            <tr>
                <td align="right" colspan="2">
                    <input type="button" name="" value="{$lng_label_prifle_photograper_button}" id="save-btn" />
                    <input type="button" name="" value="{$lng_label_prifle_photograper_change_pwd}" id="change-pwd" onclick="$('#change-pwd-dialog').dialog('open');" />
                </td>
            </tr>
        </table>
    </fieldset>
...
<div id="change-pwd-dialog" class="change-pwd-dialog">
    ...
    <table>
        <tr>
            <th>{$lng_label_prifle_photograper_old_password}</th>
            <td><input type="password" name="password" value="" id="old-password" /></td>
        </tr>
        <tr>
            <th>{$lng_label_prifle_photograper_new_password}</th>
            <td><input type="password" name="password" value="" id="new-password" /></td>
        </tr>
        <tr>
            <th>{$lng_label_prifle_photograper_confirm_password}</th>
...
{literal}
    <script type="text/javascript">
        ...
        function saveInformation(){
            ...
            $.getJSON('{/literal}{php}echo prep_url(base_url().'/profile/saveinfo');{/php}{literal}', data, function(json){
                if(json.result) {
        ...
        function savePwd(){
            ...
            $.getJSON('{/literal}{php}echo prep_url(base_url().'/profile/savepwd');{/php}{literal}', data, function(json){
                if(json.result) {
