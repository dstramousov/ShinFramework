        ...
        foreach($array_data['data'] as &$data) {
            ...
            switch(trim($data['watermarkusage'], '"')) {
                case 's':
					...
					$_tmp_arr = preg_split("/\./", SHIN_Core::$_config['gallery']['system_watermark']);
		            $data['wtm_file_name']	=   '"<img src=\'' . prep_url(SHIN_Core::$_config['core']['app_base_url'] . '/' . SHIN_Core::$_config['gallery']['photo_photographer_data'] . '/') . $_tmp_arr[0] . '_wm.'.$_tmp_arr[1]  . '\'/>"';
                    break;
                case 'c':
...
//				'watermark_status'		=> $this->watermark_status,
    ...
    * @sa SHIN_Model::read_form()
     */
    function write($fields_to_read = null)
    {
        $h = parent::write($fields_to_read);
        dump($h);
