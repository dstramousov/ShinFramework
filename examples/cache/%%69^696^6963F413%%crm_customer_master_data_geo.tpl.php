<?php /* Smarty version 2.6.26, created on 2011-03-02 18:19:53
         compiled from crm_customer_master_data_geo.tpl */ ?>
...
<body id="page">
    ...
    <fieldset>
        <legend style="font-size: 30px; font-weight: bold;">Geo customer master data form example</legend>
            ...
            <form action="" method="post" id="master_data_form">
                <table style="width: 1100px;">
                    ...
                    <tr>
                        ...
                        <td>
                            <img src="images/compassa.gif" width="30" height="30" style="display: none; cursor: pointer;" onclick="showMap()" id="compass" />
                        </td>
                    </tr>
                    <tr>
    ...
    <?php echo '
        <script type="text/javascript">
                ...
                $(\'#map-dialog\').dialog({
                    open:   function(){
                        ...
                        if(lat != 0 && lng != 0) {
                            ...
                            makeAjaxRequest({
                                action: \'get-coords\',
                                id:     $(\'#examples_crmmasterdata_idCustomer\').val()
                            }, function(json){
                            
                                if(json.lat !=0 && json.lng != 0 && json.error == \'\') {
                                    init();
                                 
                                    var location = new google.maps.LatLng(json.lat, json.lng);
                                    var marker   = new google.maps.Marker({
                                                                              position: location, 
                                                                              map:      map,
                                                                              icon:     null
                                    ...
                                    map.setCenter(location);
                                 } else if(json.error != \'OK\') {
                                    $(\'#map-dialog\').dialog(\'close\');
                                    alert(\'Google Service Error: \' + json.error[0]);
                                 }
                            })    
                        }
                    }
            ...
            function makeAjaxRequest(data, callback){
                $.ajax({
                        url:        \''; ?>
<?php echo prep_url(shinfw_base_url()); ?><?php echo '/examples/crm_customer_master_data_geo.php\',
                        dataType:   \'json\',
                        data:       data,
                        type:       \'POST\',
                        success:    callback
