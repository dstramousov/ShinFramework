...
function displayPqp($output, $config, $css_url) {

$cssUrl = $css_url.'css/pQp.css';
...
if(isset($output['queryTotals']['count'])){
...
if(isset($output['queryTotals']))
{
	if($output['queryTotals']['count'] ==  0  ) {
		...
		foreach($output['queries'] as $query) {
			echo '<tr>
				<td class="'.$class.'">'.$counter.')&nbsp;'.$query['sql'];
			...
			if(isset($query['explain'])) {
					...
					echo '<em><b>Explain</b> for this type of SQL query not support.</em>';
			}
			$counter++;
			echo '</td></tr>';
			if($class == '') $class = 'alt';
...
echo <<<FOOTER
	<table id="pqp-footer" cellspacing="0">
		<tr>
			<td class="credit">
				<a href="http://www.shinsoftware.it/" target="_blank">
				<b class="green">S</b><b class="blue">H</b><b class="red">I</b><b class="orange">N</b>
				Profiler</a></td>
			<td class="actions">
