 ...
 * @link        
 * @since        Version 0.1
 * @filesource   shinfw/libraries/SHIN_Charts_Multi.php
 */
 ...
 * @subpackage    Library
 * @author        
 * @link          shinfw/libraries/SHIN_Charts_Multi.php
 */
...
class SHIN_Charts_Multi extends SHIN_Charts
{
    ...
    * @var array
    */
    private $chartList      =   array('c2d' => "'MSColumn2D'", 
                                      'c3d' => "'MSColumn3D'", 
                                      'l2d' => "'MSLine2D'",      
                                      'a2d' => "'MSArea2D'", 
                                      'b2d' => "'MSBar2D'",
    ...
    public function combinedInit($_arr)
    {
        ...
        if(isset($_arr['yval']) && count($_arr['yval']) > 0) {
            foreach($_arr['yval'] as $key => $value){
                $data   .=  '<dataset ' . (isset($value['param']) && is_array($value['param']) ? $this->getTagAttributes('dataset', $value['param']) : $this->getTagAttributes('dataset')) . ' seriesname="' . $key . '" color="' . (isset($value['param']['color']) ? $value['param']['color'] : $this->getRandomColor()) .'">';
                    if(isset($value['data']) && count($value['data']) > 0) {
                        foreach($value['data'] as $yVal){
                            if(!isset($yVal['color'])){
                                $yVal['color'] =   $this->getRandomColor();
                            }
                            $data   .=  '<set ' . (!empty($yVal['value']) ? $this->getTagAttributes('set', $yVal) : '') . ' />';
...
/* End of file SHIN_Charts_Multi.php */
/* Location: ./libraries/SHIN_Charts_Multi.php */               