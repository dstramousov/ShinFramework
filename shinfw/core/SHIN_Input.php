<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed 2');
/**
...
class SHIN_Input
{
	...
	*/
	...
	{
		SHIN_Core::log('debug', 'SHIN_Input component initialized .');
		...
		$this->use_xss_clean	= ($config['input']['global_xss_filtering'] === TRUE) ? TRUE : FALSE;
		$this->allow_get_array	= ($config['input']['enable_query_strings'] === TRUE) ? TRUE : FALSE;
		$this->_sanitize_globals();

		Console::logSpeed('|CC| SHIN_Input begin work, Time taken to get to line: '.__FILE__.'::'.__LINE__);
		Console::logMemory($this, '|CC| SHIN_Input. Size of class: ');
	}


	function __init($params2sel=array())
	{
	   	if ( empty($params2sel) ) {
	        $this->not_touched_params = array_merge($_POST, $_GET);
	    } else {
	        $result = array();
	...
	* @return	string
	*/
	function globalarr($name)
	{
		$res = "";

	    if (isset($this->not_touched_params[$name])) {			
	        $res = $this->_clean_input_data($this->not_touched_params[$name]);
	    } else {
	        return null;
	    }
	...
	*/
	function xss_clean($str, $is_image = FALSE)
	{
		...
		*/
		...
		//!FIXME log_message('debug', "XSS Filtering completed");
		return $str;
	}
	...
	*/
	...
	public function csrf_set_cookie()
	{
		...
		SHIN_Core::log('debug', 'CRSF cookie Set.');
	...
	{
		SHIN_Core::log('debug', 'The action you have requested is not allowed.');
	}
...
// END SHIN_Input class

/* End of file SHIN_Input.php */
/* Location: ./core/SHIN_Input.php */