...
class SHIN_DB_mysqli_forge  {
	 ...
	 * @return	string
	 */
	function _process_fields($fields, $table=null)
	{
		...
		foreach ($fields as $field=>$attributes)
		{
			...
			else
			{
                ...
                switch($attributes['TYPE']) {
                    case 'enum':
						$_tmp = array();
                    	foreach(array_keys($attributes['VALUES']) as $attr){
							if(is_int($attr)){
								array_push($_tmp, "'".$attr."'");
							}else{
								array_push($_tmp, $attr);
							}
						}
						//dump($_tmp);
                        $sql .= ' ' . $attributes['TYPE'] . ' (' . implode(',', $_tmp) . ')';
				...
				// Added new attribute to create unqite fields.
	...
	{   
		...
		$sql .= $this->_process_fields($fields, $table);
...
/* End of file SHIN_DB_mysqli_forge.php */
/* Location: shinfw/core/database/drivers/mysqli/SHIN_DB_mysqli_forge.php */
