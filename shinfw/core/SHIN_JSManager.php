...
class SHIN_JSManager
{
     ...
     * @return NULL.
	 */
	private function renderCRUDObject()
	{   
		$iterator = 1;
		foreach($this->crudObjects as $obj)
		{			
			SHIN_Core::$_libs['templater']->assign('crudData_'.$iterator,  $obj."\n");
			$iterator++;
		}
    }
	
	/**
	 * Add new CRUD object in to internal array.
     *
     * @access public
     * @params param: prepared object $_crudObj.
     * @return NULL.
	 */
	private function addCRUDObject($_crudObj)
	{   
		array_push($this->crudObjects, $_crudObj);
    }
	...
	{
		Console::logMemory($this->needed_js_include, '|CC| SHIN_JSManager::renderIncludes(). Size of inserted part before render JS insjection:');
	...
	{		
		Console::logMemory($this->domready_components, '|CC| SHIN_JSManager::renderDocReady(). Size of inserted part before render JS insjection:');
	...
	$(document).ready(function(){'."\n\n";
		...
		// And add in to docready section if it`s needed initialization for crud
		$this->renderCRUDObject();
