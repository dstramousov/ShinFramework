<table width="100%" border="1" cellpadding="3" cellspacing="3">
    <tr>
        <td colspan="2" id="menulist">
            <input type="button" name="button" value="{$lng_label_create_node}" id="create" />
            <input type="button" name="button" value="{$lng_label_delete_node}" id="remove" />
            <input type="button" name="button" value="{$lng_label_copy_node}" id="copy" disabled="disabled" />
            <input type="button" name="button" value="{$lng_label_past_node}" id="paste" disabled="disabled" />
            <!--<input type="button" name="button" value="{$lng_label_create_node}" id="" />-->
        </td>
    </tr>
    <tr>
        <!-- left section -->
        ...
        <td width="30%">
            ...
            <table border="1">
                <tr>
                    <th>{$lng_label_question_panel_name}</th>
                </tr>
                <tr>
                    <td>
                        <table id="question_info">
                            <tr>
                                <th>{$lng_label_question_description}</th>
                                <td>
                            ...
                            <tr>
                                <th>{$lng_label_question_img_old}</th>
                                <td id="question_img"></td>
                            </tr>
                            <tr>
                                <th>{$lng_label_question_img_new}</th>
                                <td>
            ...
            <table border="1">
                <tr>
                    <th>{$lng_label_answer_panel_name}</th>
                </tr>
                <tr>
                    <td>
                        <table id="answer_info">
                            <tr>
                                <th>{$lng_label_answer_description}</th>
                                <td>
                            ...
                            <tr>
                                <th>{$lng_label_answer_img_old}</th>
                                <td id="answer_img"></td>
                            </tr>
                            <tr>
                                <th>{$lng_label_answer_img_new}</th>
                                <td>
                            ...
                            <tr>
                                <th>{$lng_label_answer_solution}</th>
                                <td>{$answer_solution_custom_input}</td>
                            </tr>
                            <tr>
                                <th>{$lng_label_answer_userinsert}</th>
                                <td><input type="text" value="" name="answer_userinsert" id="answer_userinsert" disabled="disabled" /></td>
                            </tr>
                            <tr>
                                <th>{$lng_label_answer_datainsert}</th>
                                <td><input type="text" value="" name="answer_datainsert" id="answer_datainsert" disabled="disabled" /></td>
                            </tr>
                            <tr>
                                <td align="right" colspan="2"><input type="button" value="{$lng_label_save_answer_submit}" name="" id="save_answer" /></td>
                            </tr>
                        </table>
                    </td>
                </tr>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#menulist input").click(function () {
                switch(this.id) {
                    case "create":
                        $("#gtrwebsite_tree_dom_structure").jstree("create", null, "last", '{/literal}{$lng_label_new_node}{literal}', false, true);
                        break;
                    default:
            ...
            $('#save_question').bind('click', function(){
                ...
                if(selectedNode == "" || selectedNode == undefined) {
                    alert('{/literal}{$lng_label_node_not_selected_prompt}{literal}');
                    return false;
                }
                
                $.ajax({
                    url:        '{/literal}{php}echo prep_url(base_url().'/treemanagment/validateq');{/php}{literal}',
                    dataType:   'json',
                    type:       'POST',
                    data:       {
                    ...
                    success: function(json) {
                        if(json.result) {
                            ...
                            // create question
                            $.ajax({
                                url:        '{/literal}{php}echo prep_url(base_url().'/treemanagment/createq');{/php}{literal}',
                                dataType:   'json',
                                type:       'POST',
                                data:       {
                                    gtrwebsite_question_idQuestion:     $('#gtrwebsite_question_idQuestion').val(),
            ...
            $('#save_answer').bind('click', function(){
                ...
                $.ajax({
                    url:        '{/literal}{php}echo prep_url(base_url().'/treemanagment/validatea');{/php}{literal}',
                    dataType:   'json',
                    type:       'POST',
                    data:       {
                    ...
                    success: function(json) {
                        if(json.result) {
                            ...
                            // create answer
                            $.ajax({
                                url:        '{/literal}{php}echo prep_url(base_url().'/treemanagment/createa');{/php}{literal}',
                                dataType:   'json',
                                type:       'POST',
                                data:       {
